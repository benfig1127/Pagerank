...
def url_satisfies_query(url, query):
	...
	if query!='':
		most_similar_words=vectors.most_similar(query)
	#print(most_similar_words)
		most_similar_words_list=[]
		for tuplle in most_similar_words:
			most_similar_words_list.append(list(tuplle)[0])
	#print(most_similar_words_list)
		for similar_word in most_similar_words_list:
			if similar_word in url:
				satisfies=True
				break
			else:
				satisfies=False
