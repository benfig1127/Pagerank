...
class WebGraph():
	...
	def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
		...
		with torch.no_grad():
			...
			a= torch.ones(n)
			v=torch.unsqueeze(v,1)
			row_sum_vals=torch.sparse.sum(self.P,1)
			...
			for i in range(max_iterations):
				x_k_minus_one=x
				x=(torch.sparse.mm(self.P.t(),(alpha*x_k_minus_one.t()).t()).t() + ((((alpha*x_k_minus_one.t())*a)+(1-alpha))*v.t())).t()
				
				if abs(torch.norm(x)-torch.norm(x_k_minus_one))<=epsilon:
					break 
			return x.squeeze()
